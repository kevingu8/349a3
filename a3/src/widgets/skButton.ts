...
import { requestMouseFocus } from "simplekit/imperative-mode";
...
export class SKButton extends SKElement {
  ...
  state: "idle" | "hover" | "down" | "disabled" = "idle";
  ...
  draw(gc: CanvasRenderingContext2D) {
    ...
    if (this.state === "disabled") {
      ...
      // normal background
      gc.fillStyle = this.state == "down" ? Style.highlightColour : this.fill;
    ...
    // button label
    gc.font = this._font;
    gc.fillStyle = this.state === "disabled"? "darkgrey" : this.border;
    gc.textAlign = "center";
